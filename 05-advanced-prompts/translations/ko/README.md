...
- 출력을 다양화하도록 프롬프트를 구성합니다.

## 학습 목표

이 레슨을 완료한 후에는 다음을 할 수 있게 될 것입니다:

- 프롬프트 엔지니어링 기법을 적용하여 프롬프트의 결과를 개선할 수 있습니다.
- 다양하거나 결정론적인 프롬프트를 수행할 수 있습니다.

## 프롬프트 엔지니어링

프롬프트 엔지니어링은 원하는 결과를 얻을 수 있는 프롬프트를 생성하는 과정입니다. 이는 단순히 텍스트 프롬프트를 작성하는 것 이상의 의미를 가집니다. 프롬프트 엔지니어링은 하나의 엔지니어링 학문이라기보다는, 원하는 결과를 얻기 위해 적용할 수 있는 기술의 집합입니다.
...
원하는 결과를 얻을 수도 있고, 얻지 못할 수도 있습니다. 질문은 생성되지만, 질문은 큰 주제이며 원하는 내용을 얻지 못할 수 있습니다. 이는 다음과 같은 이유로 인해 발생할 수 있습니다:

- **큰 주제**: 국가, 수도, 강 등에 대한 질문인지 알 수 없습니다.
- **형식**: 질문을 특정한 방식으로 포맷하고 싶은 경우, 어떻게 해야 할까요?
...
먼저, 프롬프트는 LLM의 _내재된_ 특성이라는 점을 이해해야 합니다. 이는 모델에 내장된 기능이 아니라 모델을 사용하면서 우리가 발견하는 것입니다.
...
- **작은 것부터 크게 (Least to most)**, 생각의 사슬과 마찬가지로, 이 기법은 문제를 일련의 단계로 분해하고 이 단계들을 순서대로 수행하도록 요청하는 것입니다.
...
이 프롬프트 스타일은 매우 간단하며, 단일 프롬프트와 몇 가지 예제로 구성될 수 있습니다. 이 기법은 LLM에 대해 배우기 시작하는 단계에서 사용할 수 있습니다. 다음은 예시입니다:
...
Chain-of-thought는 LLM을 일련의 단계를 거쳐 가르치는 매우 흥미로운 기술입니다. 이 아이디어는 LLM에게 문제를 어떻게 해결할지 이해시키는 방법입니다. 다음 예시를 고려해보세요. Chain-of-thought를 사용하는 경우와 그렇지 않은 경우를 비교해보세요:
...
보시다시피, 이는 생명 보험도 제안하는데, 그렇게 되면 안 됩니다. 이 결과는 우리가 원하는 결과를 얻기 위해 프롬프트를 최적화해야 함을 나타냅니다. 시행착오를 거쳐 다음과 같은 프롬프트로 도달합니다:
...
예산: $1000, 자동차 및 주택 유형으로 선택 범위 제한
```

_유형_과 _비용_을 추가하고 _제한_ 키워드를 사용하는 것이 어떻게 우리가 원하는 바를 LLM에게 이해시키는 데 도움이 되는지 주목하세요.
...
이 예시의 목적은 _Generated knowledge_과 같은 기본적인 기술을 사용하더라도 대부분의 경우 원하는 결과를 얻기 위해 프롬프트를 최적화해야 한다는 것을 보여주는 것이었습니다.
...
Maieutic prompting은 자기 개선 (Self-refine)과 유사한 기술이지만, LLM에게 스스로를 설명하도록 요청하는 것입니다. 목표는 LLM의 출력에서 일관성을 높여 올바른 답변에 도달하는 것입니다. 따라서 다음과 같은 워크플로우를 따릅니다:
...
A: 1, 생각의 사슬 (chain-of-thought)은 LLM에게 문제를 해결하는 방법을 보여주는 것으로, 일련의 단계와 유사한 문제 및 그들이 어떻게 해결되었는지를 제공합니다.

## 🚀 도전과제

과제에서 자기 개선 (self-refinement) 기법을 사용했습니다. 구축한 프로그램을 가져와서 적용할 개선 사항을 고려해보세요. 그리고 자기 개선 기법을 사용하여 제안된 변경 사항을 적용해보세요. 결과는 어땠나요? 더 좋아졌나요 아니면 더 나빠졌나요?
...
[6번 레슨](../../../06-text-generation-apps/translations/ko/README.md?WT.mc_id=academic-105485-koreyst)으로 이동하여 프롬프트 엔지니어링 지식을 적용하여 텍스트 생성 앱을 구축해보세요.